...
// Workaround for default marker icon broken images in Leaflet + React
      ...
      {selectedMapProperty && (
        <div className="absolute bottom-4 left-4 right-4 z-[1000] bg-[#1C3A27]/95 backdrop-blur-md border border-white/10 rounded-2xl shadow-2xl p-4 text-white flex flex-col sm:flex-row items-stretch gap-4 pointer-events-auto">
          <div className="flex-1 flex flex-col justify-between">
            <div>
              ...
              <h4 className="font-serif text-sm font-bold text-white leading-tight">
                {selectedMapProperty.title ?? selectedMapProperty.name ?? 'Property'}
              </h4>
            ...
            <div className="flex items-center gap-3 text-[11px] text-slate-300 mt-2">
              <span className="flex items-center gap-1 font-semibold">
                ...
                {(selectedMapProperty.size ?? selectedMapProperty.lotSize ?? 0).toLocaleString()} sqm
              </span>
          ...
          <div className="flex flex-row sm:flex-col justify-between items-end gap-2 shrink-0">
            ...
            <button className="w-full sm:w-auto px-4 py-2 text-[11px] font-bold text-white bg-emerald-600 hover:bg-emerald-500 rounded-xl shadow-lg transition-all flex items-center justify-center gap-1 cursor-pointer active:translate-y-0.5">
