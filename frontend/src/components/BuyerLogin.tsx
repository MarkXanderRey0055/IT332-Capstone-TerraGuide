...
const BuyerLogin: React.FC<LoginProps> = ({ onBack, onSuccess }) => {
  ...
  const handleSubmit = (event: React.FormEvent) => {
    ...
    if (isSignUp) {
      ...
      setIsSignUp(false);
      setFeedback({ type: 'success', message: 'Account created successfully. Please sign in with your new account.' });
      return;
    }
    ...
    if (!matchedAccount) {
      setFeedback({ type: 'error', message: 'No account found with those credentials.' });
      return;
    }

    localStorage.setItem(currentUserKey, JSON.stringify({ username: matchedAccount.username, email: matchedAccount.email }));
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#112a1d] p-4 relative overflow-hidden font-sans">
      ...
      <div className="w-full max-w-md p-10 rounded-[2.5rem] border border-white/10 bg-white/[0.03] backdrop-blur-md shadow-2xl relative z-10 text-left">
        ...
        {feedback.message ? (
          <div className={`mb-4 rounded-xl border px-3 py-2 text-sm ${feedback.type === 'error' ? 'border-red-500/30 bg-red-500/10 text-red-200' : 'border-emerald-500/30 bg-emerald-500/10 text-emerald-200'}`}>
            {feedback.message}
          </div>
        ) : null}
