...
// @desc    Get trending property types (from Buyer Preferences) and the
//          top 5 listings by existing marketReadinessScore, for the
//          Buyer Home page's compact "market intelligence" section
// @route   GET /api/analytics/buyer/market-trends
...
// @desc    Generate an AI buyer market insight report
// @route   POST /api/analytics/buyer/market-insight
// @access  Private (Buyer)
...
// @desc    Get recent Dashboard activity, derived at read-time from
//          Property/User/Transaction — no separate activity log
// @route   GET /api/analytics/activity
// @access  Private (Admin)
export const getRecentActivity = asyncHandler(async (req, res) => {
  const activity = await activityService.getRecentActivity();

  return sendSuccess(res, 200, 'Recent activity retrieved successfully.', activity);
});
